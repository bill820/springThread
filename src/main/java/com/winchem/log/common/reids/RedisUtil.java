package com.winchem.log.common.reids;
...
public class RedisUtil<T> {
    ...
    /**
    *@Description: 根据key通配符删除缓存
    *@Param:  * @param null :
    *@return:  * @return : null
    *@Author: zhanglb
    *@date: 2020/11/3 15:56
    */
    public void delByRegix(String keyPattern) {
     ...
     *@date: 2020/11/3 15:56
     */
    public List<T> getByRegix(String keyPattern) {
        Set<String> keys = redisTemplate.keys(keyPattern);
        List<T> lis = new ArrayList<T>();
        for (String key : keys) {
            lis.addAll(this.lGetAll(key));
        }
        return lis;
    }
    ...
    public boolean lSet(String key, List<Object> value) {
        if (CollectionUtils.isEmpty(value)) {
            log.warn("时间:" +DateUtils.getDateTime()+ "将list放入缓存: 参数list为空值");
            return true;
        }
        try {
